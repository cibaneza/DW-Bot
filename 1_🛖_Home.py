...
from sql_execution import execute_bq_query
...
# frontend
st.set_page_config(page_title="Callpiwis Query Assistant", page_icon="🌄")
st.sidebar.success("Select a page above")

tab_titles = ["Results", "Query", "ER Diagram"]

st.title("Callpiwis Bot Assistant 🤖")
...
if prompt:
    ...
    output = execute_bq_query(query_text)
    with tabs[0]:
        st.write(output)
    ...
    with tabs[1]:
        ...
        if add_to_training_data:
            ...
            write_to_file_status = write_to_training_file(
                file_path=file_path, prompt=prompt, sql=query_text
            )
